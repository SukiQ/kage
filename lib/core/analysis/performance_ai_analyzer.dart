...
class PerformanceAiAnalyzer {
  ...
  Future<PerformanceReport> analyze({
    ...
    try {
      ...
      // 兜底：进程异常退出且未发 result 事件时，解除等待以便报错。
      proc.done.then((_) {
        if (!doneCompleter.isCompleted) doneCompleter.complete();
      });

      final prompt = AnalysisPrompt.buildPerformanceReportMessage(
        projectName: project.name,
        projectPath: project.path,
      );
      await proc.send(prompt);
      await doneCompleter.future.timeout(timeout, onTimeout: () {
        throw TimeoutException('AI 性能分析超时（${timeout.inMinutes} 分钟）。');
      });
    } finally {
      await sub?.cancel();
    ...
    if (decoded == null) {
      ...
      throw Exception('无法从 AI 输出中解析性能报告 JSON。\n原始输出片段：\n$preview');
    }
    return PerformanceReport.fromJson(decoded);
  }
